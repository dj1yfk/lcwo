...
function load(lang, collid, collection, g_filter) {
    ...
    var request =  new XMLHttpRequest();
    request.open("GET", '/api/index.php?action=get_wordtraining_collection&id=' + l + '&filter=' + g_filter, true);
    request.onreadystatechange = function() {
            var done = 4, ok = 200;
            if (request.readyState == done && request.status == ok) {
                    if (request.responseText) {
                        ...
                        o += 'Filter: <input type="text" id="filter" onkeyup="filter_list(this.value);" value="' +  g_filter + '"><br>'
