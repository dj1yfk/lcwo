...
$gc = mysqli_query($db,"select `max`, `score` from lcwo_callsignsresults where `uid`='$_SESSION[uid]' order by nr");
...
graph($width, $height, $score, array(0,max($score)), 
						$max, array(0,max($max)),
						array(), "calls", array("Score",
						"Speed", '', l('wpm')));


?>

<img src="/img/calls<? echo imgurl($_SESSION['uid']); ?>.gif">
